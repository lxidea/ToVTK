/*
 <TOVTK4>  Copyright (c) 2020, Dr Jose M. Dominguez et al. (see http://dual.sphysics.org/index.php/developers/). 
...
void JCfgRun::LoadOpts(std::string *optlis,int optn,int lv,std::string file){
  ...
  for(int c=0;c<optn;c++){
    ...
    if(opt[0]=='-'){
      ...
      else if(txword=="SAVEGRID"&&c+1<optn){ SaveGrid=optlis[c+1]; c++; }
      else if(txword=="OPT"&&c+1<optn){ LoadFile(optlis[c+1],lv+1); c++; }
      else if(txword=="CONTINUE"){ Continue=true;c++;}
...
void JCfgRun::ValidaCfg(){
  ...
  if(SaveVtk.empty()&&SaveCsv.empty()&&SaveGrid.empty())RunException(met,"Output files were not defined.");
}
