...
void JDataArrays::FreeMemory(StDataArray &arr){
  if(arr.delptr){
    ...
    switch(arr.type){
      ...
      case TypeUint8:    delete[] (tuint8  *)ptr;  break;
...
unsigned JDataArrays::GetDataCount()const{ 
  ...
  if(smin!=smax)Run_ExceptioonSta(fun::PrintStr("The minimum data count does not match the maximum data count. %u!=%u",smin,smax));
  return(smin);
}
...
//==============================================================================
/// Returns dynamic pointer with tuint8 array. (this pointer must be deleted)
//==============================================================================
tuint8* JDataArrays::NewArrayUint8(unsigned count,bool defvalue,tuint8 value){
  try{
    tuint8 *v=new tuint8[count];
    if(count && defvalue){
      if(value==TUint8(0))memset(v,0,sizeof(tuint8)*count);
      else for(unsigned c=0;c<count;c++)v[c]=value;
    }
    return(v);
  }
...
unsigned JDataArrays::FilterApply(unsigned count,const byte *filter){
  ...
  for(unsigned c=0;c<na;c++){
    ...
    switch(arr.type){
      ...
      case TypeUint8:    ReindexData(nsel,reindex,arr.count,(tuint8  *)arr.ptr,NULL);  break;
      default: Run_Exceptioon(fun::PrintStr("Type of pointer \'%s\' is invalid.",TypeToStr(arr.type)));
    }
    arr.count=nsel;
  }
...
unsigned JDataArrays::SortData(unsigned count,const unsigned *reindex){
  ...
  for(unsigned c=0;c<na;c++){
    ...
    switch(arr.type){
      ...
      case TypeUint8:    ReindexData(count,reindex,arr.count,(tuint8  *)arr.ptr,(tuint8  *)aux);  break;
      default: Run_Exceptioon(fun::PrintStr("Type of pointer \'%s\' is invalid.",TypeToStr(arr.type)));
    }
    arr.count=count;
  }
